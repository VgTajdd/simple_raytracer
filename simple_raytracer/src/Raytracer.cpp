  ...
  for (unsigned int r = 0; r < m_height; r++) {
    for (unsigned int c = 0; c < m_width; c++) {
      Ray ray(m_eyeLocation,
              Vec3(c + 0.5f - m_width / 2, (m_height / 2) - (r + 0.5f), 0) -
                  m_eyeLocation);
...
Vec3 Raytracer::computeLight(const Vec3& direction, const Vec3& lightcolor,
...
Vec3 Raytracer::computeColor(const Scene& scene, const Ray& ray,
  ...
  if (intersection.object != nullptr) {
    for (const auto& light : scene.lights()) {
      // Shadows section.
      {
        ...
        Ray rayToLight(intersection.pi + directionToLight * 0.01f,
                       directionToLight);
      ...
      Vec3 color = computeLight(direction, light.color, intersection.normal,
                                halfVec, intersection.object->material());
      pixelColor = pixelColor + color;
    }
    pixelColor = intersection.object->material().ambient +
                 intersection.object->material().emissive + pixelColor;

    // Reflection section.
    if (maxDepth > 0) {
      auto a = intersection.normal * intersection.normal.dot(ray.direction());
      auto reflectedDirection = ray.direction() - a * 2;
      Ray reflectedRay(intersection.pi + reflectedDirection * 0.01f,
                       reflectedDirection);
      IntersectionInfo intersectionFromReflectedRay =
          intersect(scene, reflectedRay);
      if (intersectionFromReflectedRay.object != nullptr) {
        Vec3 reflectedColor;
        reflectedColor =
            computeColor(scene, reflectedRay, intersectionFromReflectedRay,
                         maxDepth - 1) *
