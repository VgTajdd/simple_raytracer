...
struct Light {
  explicit Light(const Vec3& pos, const Vec3& col, const bool point)
      : position(pos), color(col), isPoint(point) {}
  Vec3 position;
  Vec3 color;
  bool isPoint;
};
...
class SceneObject {
 ...
 public:
  explicit SceneObject(const unsigned int objectId, const OBJ_TYPE& type)
      : m_objectId(objectId), m_type(type) {}
...
class Sphere : public SceneObject {
 public:
  explicit Sphere(const Vec3& center, const float radious,
...
class Triangle : public SceneObject {
 public:
  explicit Triangle(const Vec3& v1, const Vec3& v2, const Vec3& v3,
                    const unsigned int id)
      : SceneObject(id, OBJ_TYPE::TRIANGLE), m_v1(v1), m_v2(v2), m_v3(v3) {
...
class ObjectsFactory {
 public:
  ...
  inline static Triangle* createTriangle(const Vec3& v1, const Vec3& v2,
                                         const Vec3& v3) {
